...
export const CustomCrosshairPreview = ({ crosshair, className = '' }: CustomCrosshairPreviewProps) => {
	const color = getCrosshairPreviewColor(crosshair);
	const activeColor = `rgb(${color.r}, ${color.g}, ${color.b})`;

	return (
		<Card className={`overflow-hidden border-white/10 bg-card/75 p-0 shadow-2xl shadow-black/25 backdrop-blur-xl ${className}`}>
			...
			<div className="p-4">
				<div className="relative flex aspect-[16/10] items-center justify-center overflow-hidden rounded-lg border border-white/10 bg-[linear-gradient(135deg,#d7b886,#8b7355_62%,#3d352c)]" role="img" aria-label="Custom crosshair preview">
